...
@Component({
  selector: 'osb-tax-others',
  templateUrl: './tax-others.component.html'
})
export class TaxOthersComponent implements OnInit {
  ...
  createFormControl() {
    this.newOtherForm = new FormGroup ({
      accountId: new FormControl('', Validators.required),
      taxPaymentClass: new FormControl('OthersTaxPayment'),
  ...
  makeOperation() {
    ...
    this.accountSelected = this.accounts.find(account => account.id === this.newOtherForm.value.accountId);
    this.taxFormSelected = this.taxForms.find(taxForm => taxForm.id === this.newOtherForm.value.taxForm);
    this.periodSelected = this.periods.find(period => period.id === this.newOtherForm.value.taxPeriod);
  ...
  createObj(): IvaTaxPayment {
    ...
    _newIvaTaxPayment.accountId = this.newOtherForm.value.accountId;
    _newIvaTaxPayment.taxPaymentClass = this.newOtherForm.value.taxPaymentClass;
    _newIvaTaxPayment.taxForm = this.taxForms.find(form => form.id === this.newOtherForm.value.taxForm);
    _newIvaTaxPayment.type = this.newOtherForm.value.type;
    _newIvaTaxPayment.taxYear = this.newOtherForm.value.taxYear;
    _newIvaTaxPayment.taxPeriod = this.newOtherForm.value.taxPeriod;
    _newIvaTaxPayment.paymentAmount = { amount: this.newOtherForm.value.paymentAmount, currency: null };
