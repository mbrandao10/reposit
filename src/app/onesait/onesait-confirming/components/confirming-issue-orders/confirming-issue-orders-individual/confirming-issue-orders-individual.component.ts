...
import { ConfirmingService, ModalPageComponent, DeviceUtilsService, AppConfigService, InputValidatorOptions } from 'onesait-core';
import { ConfirmingContracts, ConfirmingSuppliers, ConfirmingOrder, ConfirmingNewSupplier, CurrenciesCredits } from 'onesait-api';
...
@Component({
  selector: 'osb-confirming-issue-orders-individual',
  templateUrl: './confirming-issue-orders-individual.component.html'
})
export class ConfirmingIssueOrdersIndividualComponent implements OnInit {
  ...
  suppliers: ConfirmingSuppliers[];
  newSupplier: ConfirmingNewSupplier;
  ...
  constructor(
    ...
    this.newOrderObject = new ConfirmingOrder();
  ...
  loadInitialData() {
    ...
    observableData.push(this.confirmingService.getContracts());
    // observableData.push(this.confirmingService.getSuppliers());
  ...
  issueOrder() {
    // this.newOrderObject.amount.amount = this.amountOrder;
  ...
  getValueSelectedSupplier(event: any) {
    console.log('id: ' + event.frmCntrlAcutocomplete.get('id').valid);
    console.log('name: ' + event.frmCntrlAcutocomplete.get('name').valid);
